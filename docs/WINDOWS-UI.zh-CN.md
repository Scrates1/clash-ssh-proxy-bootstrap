...
网页已经关闭，则重新打开带有效会话的界面。后台在本次 Windows 登录会话期间保持
运行，关闭、最小化或休眠网页不会使后台因缺少心跳而在 90 秒后退出。后台已退出时，
入口会自动启动新实例。
...
无法连接本地管理器时，页面保留上次成功获取的数据并显示恢复方法。这表示与 Windows
本地后台的连接失败；Linux 的连接状态请查看 SSH 和 Proxy 列。心跳恢复、页面重新
获得焦点或变为可见、网络恢复时，界面会自动刷新。如果重试仍失败，请通过
`Open-ProxyManager.vbs` 重新打开；刷新旧页面不能重新启动后台，也不能取得后台
重启后的新会话。
