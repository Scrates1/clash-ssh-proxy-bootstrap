...
- React 会话令牌通过 URL fragment 交给前端，读取后保存在当前标签页的 sessionStorage
  （受限环境回退到当前 history state），再从地址栏移除；页面刷新可以恢复授权，关闭标签页
  后不会形成长期凭据。API 同时校验会话令牌、同源来源和请求体大小，并为所有响应设置
  限制脚本、嵌入与引用来源的安全头。
  Smoke test 会发起真实的静态资源、鉴权与同源请求，验证监听器边界而非只检查源码；
  Edge E2E 使用空的临时配置和独立互斥锁，验证首次加载、手动刷新、令牌脱敏与页面重载。
  请求大小限制由独立的行为测试覆盖。
- 密码只能存在于明确打开的交互式 SSH 控制台，不能进入参数对象、日志或配置文件。
- 单个回环 HTTP 后台在 Windows 登录会话期间保持运行，不把浏览器暂停心跳视为
  用户关闭页面。页面在心跳恢复、获得焦点、重新可见或网络恢复时刷新快照，心跳请求
  不重叠。网络错误与会话失效有中英文恢复说明。生命周期回归通过独立宿主时钟模拟
  两分钟和两小时的心跳中断，验证旧会话仍可使用；不触及生产窗口或配置。
- 新目标使用稳定的 `tgt-...` ID；管理器为每个 ID 派生独立的 Ed25519 私钥路径。
  `host + user` 是目标唯一性，SSH 端口只是连接参数；目标之间不得共享私钥路径。
  已有目标的主机与账号不能直接修改，CLI 构造目标及安装入口都会在副作用前拒绝；
  更换身份必须先删除旧目标再新增。主机比较忽略大小写，Linux 账号比较区分大小写。
- 静态资源按原始请求路径解码并校验，非法路径返回 400。单次请求处理异常会记录日志
  并返回 500，不使监听器退出。额外 NO_PROXY 显式空值与未传参数分别表示清空与保留。
- 远端命令与探测统一把脚本编码后交给 `sh`，避免 PowerShell 5.1 原生参数传递丢失引号。
  健康检查使用当前目标配置中的端口并强制经过代理，不受 NO_PROXY 和旧远端脚本影响。
  本地 HTTPS、有效与无效代理及远端公钥清理由 `tests/test-proxy-health.sh` 覆盖。
...
2. `proxy-manager.ps1` 的 `prepare-ssh` 只编排静默准备和预检，`bootstrap-key` 承担必要的
   一次性交互安装；入口不包含密钥处理细节。
3. React 新增目标和已有目标 SSH 修复向导位于 `web/src/App.tsx`；根据预检结果调用
   React 主机启动 `bootstrap-key` 的可交互 PowerShell 控制台。已有目标修复完成后只
   显示登录就绪，不执行安装或改变启用状态；中英文浏览器回归覆盖失败重试与成功提示。
4. 同一目标已有 SSH 配置窗口时会激活并复用该进程，不再并行创建多个密码窗口。
5. 已能免密登录时不创建控制台；只有远端尚未接受公钥时，才要求用户输入一次 Linux
   密码。密码不会进入参数对象、日志或配置。
6. 缺失密钥、已有密钥、首次交互和失败清理由 `tests/Test-Manager.ps1`、
